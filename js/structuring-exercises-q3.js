var poundConverter={
	...
	getConversionRate:function(currency){
		var selectedRate = this.conversionRates[currency];
		return selectedRate;
	},
	...
	convertPounds:function(amount, currency){
		...
		{
			var selectedRate = this.getConversionRate(currency);
			if(selectedRate===undefined)
			{
				throw new Error("Can only convert dollars or euros");
			}else{
				return this.convert(amount, selectedRate)
			}
		}
	}
}
...
console.log(poundConverter.convertPounds(10,"euros"));
